...
static int make_compressed_planets_file(const char *const file_name, const int n, bool metadata)
{
  ...
  uint8_t test_data[PlanetsHdrSize +
                    ((PaddingSize + PlanetBitmapSize) * 2 * 2)] = {0};
...
static void check_planets_file(const void *const test_data, const int n)
{
  ...
  for (int image = 0; image < 4; ++image)
    assert(reader_fread_int32(&offsets[image], &reader));

  for (int j = 0; j < n; ++j)
  {
    uint8_t p = (uint8_t)(PlanetMagic +
      j * SFPlanet_Height * (SFPlanet_Width - PlanetBorder));
    for (int image = 0; image < 2; ++image)
    {
      ...
      for (int y = 0; y < SFPlanet_Height; ++y)
      {
        for (int x = 0; x < SFPlanet_Width; ++x)
        {
          bool const margin = image == 0 ?
            x >= SFPlanet_Width - PlanetBorder : x < PlanetBorder;
          int const got = reader_fgetc(&reader);
          int const expected = margin ? 0 : q++;
          DEBUGF("image %d y %d x %x expected %d got %d\n",
                 image, y, x, expected, got);
          assert(got == expected);
        }
      }
    }
...
static void check_uncompressed_planets_file(const char *const file_name, const int n, bool metadata)
{
  ...
  if (metadata)
  {
    ...
    int32_t noffsets;
    assert(reader_fread_int32(&noffsets, &reader));
    assert(noffsets == n);
    int32_t x[2], y[2];
  ...
  for (int j = 0; j < n; ++j)
  {
    ...
    char tmp[13] = {'\0'};
    assert(reader_fread(tmp, sizeof(tmp) - 1, 1, &reader) == 1);
    int tileno;
    int const l = sscanf(tmp, "planet_%d", &tileno);
    assert(l == 1);
    assert(tileno == j);
...
static void check_uncompressed_sprites_file(const char *const file_name, const int n, bool metadata)
{
  ...
  for (int j = 0; j < n; ++j)
  {
    ...
    assert(reader_fread(tmp, sizeof(tmp) - 1, 1, &reader) == 1);
    int tileno;
    int const l = sscanf(tmp, "tile_%d", &tileno);
    assert(l == 1);
    assert(tileno == j);
    const int nwords = (SFMapTile_Width + WORD_SIZE - 1) / WORD_SIZE;
    assert(reader_fread_int32(&value, &reader));
...
static void init_radiobutton_event(WimpPollBlock *poll_block,
                                   ComponentId const old_on_button)
{
...
static int init_ram_transmit_msg(WimpPollBlock *poll_block,
                                 const WimpMessage *ram_fetch, const char *data,
                                 int nbytes)
{
...
static int init_data_save_msg(WimpPollBlock *poll_block, int estimated_size,
                              int file_type,
                              const WimpGetPointerInfoBlock *pointer_info,
                              int your_ref)
{
