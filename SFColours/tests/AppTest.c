...
static int init_data_open_msg(WimpPollBlock *poll_block, char *filename,
                              int file_type,
                              const WimpGetPointerInfoBlock *pointer_info)
{
  *poll_block = (WimpPollBlock){
        ...
        .hdr =
          {
            ...
            .my_ref = ++fake_ref,
            .your_ref = 0,
            .action_code = Wimp_MDataOpen,
          },
        .data.data_open =
          {
            .window_handle = pointer_info->window_handle,
            .padding1 = pointer_info->icon_handle,
            .x = pointer_info->x,
            .y = pointer_info->y,
            .padding2 = 0,
            .file_type = file_type,
  ...
  for (limit = 0; limit < FortifyAllocationLimit; ++limit)
  {
    ...
    switch (method)
    {
      ...
      case DTM_BadRAM:
      {
        ...
        FILE *f = test_fopen(TEST_DATA_OUT, "wb");
        int total_bytes = 0;
  ...
  for (unsigned int nwin = 0; nwin <= MaxNumWindows; ++nwin)
  {
    ...
    assert(!pseudo_toolbox_object_is_showing(prequit_id));
    {
      ...
      for (limit = 0; limit < FortifyAllocationLimit; ++limit)
      {
        ...
        dispatch_event_with_error_sim(Wimp_EUserMessage, &poll_block,
                                      limit /* wait for about-to-be-shown */);
...
static void test39(void)
{
  ...
  for (limit = 0; limit < FortifyAllocationLimit; ++limit)
  {
    ...
    Fortify_SetNumAllocationsLimit(limit);
    _Optional const _kernel_oserror * const err =
