  ...
  IdBlock *const id_block, void *const handle)
{
  ...
  DEBUGF("Mouse click handler to hide dboxes, self=0x%x parent=0x%x\n",
         id_block->self_id, id_block->parent_id);

  if (id_block->self_id != Interpolate_sharedid &&
      id_block->parent_id != Interpolate_sharedid)
    ON_ERR_RPT(DeIconise_hide_object(0, Interpolate_sharedid));

  if (id_block->self_id != Insert_sharedid &&
      id_block->parent_id != Insert_sharedid)
    ON_ERR_RPT(DeIconise_hide_object(0, Insert_sharedid));
  ...
  IdBlock *const id_block, void *const handle)
{
  ...
  switch (event_code)
  {
    case EventCode_NewFile:
      (void)SkyFile_create(NULL, NULL, true);
      break;
  ...
  IdBlock *const id_block, void *const handle)
{
  ...
  DEBUGF("Toolbox error %x '%s'\n", totee->errnum, totee->errmess);
...
void initialise(void)
{
  ...
  flex_init(taskname, NULL, 0); /* (heap in application space and default
                                    English messages) */
  ...
  /* Read the default palette for game screen mode */
