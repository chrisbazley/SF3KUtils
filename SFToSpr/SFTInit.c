...
static int data_open_msg(WimpMessage *const message, void *const handle)
{
  ...
  if (message->data.data_open.file_type != FileType_SFMapGfx &&
      message->data.data_open.file_type != FileType_SFSkyPic)
  {
  ...
  if (!E(wimp_send_message(Wimp_EUserMessage, message, message->hdr.sender,
                           0, NULL)))
  {
    quick_view(message->data.data_open.path_name,
               message->data.data_open.file_type);
  }
...
static int misc_tb_event(const int event_code, ToolboxEvent *const event,
...
static int object_auto_created(const int event_code, ToolboxEvent *const event,
  IdBlock *const id_block, void *const handle)
{
  ...
  _Optional const ObjectInitInfo *match = bsearch(toace->template_name, auto_created, ARRAY_SIZE(auto_created),
                  sizeof(auto_created[0]), compare_init_info);
  if (match != NULL)
  {
...
static int toolbox_error(const int event_code, ToolboxEvent *const event,
  IdBlock *const id_block, void *const handle)
{
...
void initialise(void)
{
  static int wimp_messages[] = {
        Wimp_MDataOpen, Wimp_MDataSave, Wimp_MDataSaveAck,
        Wimp_MDataLoad, Wimp_MDataLoadAck,
