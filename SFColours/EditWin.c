...
static EditWin *for_each_view(ColMapFile *const file,
...
{
  for_each_view(file, file_cancel_io_cb, NULL);
}
...
static EditWin *add_view(ColMapFile *const file)
{
  assert(file != NULL);
...
ColMapFile *ColMapFile_find_by_file_name(char const *const load_path)
{
...
ColMapFile *ColMapFile_create(Reader *const reader,
  char const *const load_path, bool const is_safe, bool const hillcols)
{
  ColMapFile *const file = malloc(sizeof(*file));
  if (file == NULL)
  {
    RPT_ERR("NoMem");
    return NULL;
...
void ColMapFile_destroy(ColMapFile *const file)
{
  if (file)
  {
